...
const TMETA = {
  staleMinutes: {
    l: "Stale (min)",
    g: "Connectivity",
    d: "Warn if no data this many minutes",
  },
  offlineHours: {
    l: "Offline (hours)",
    g: "Connectivity",
    d: "Critical if no data this long",
  },
  rssiWarn: {
    l: "RSSI warn (dBm)",
    g: "Connectivity",
    d: "Signal strength warning",
  },
  rssiCrit: {
    l: "RSSI critical (dBm)",
    g: "Connectivity",
    d: "Signal strength critical",
  },
  intervalDriftX: {
    l: "Interval drift X",
    g: "Connectivity",
    d: "Flag gap > Nx median",
  },
  ...
  iaqCrit: {
    l: "IAQ critical",
    g: "Environment",
    d: "Air quality index critical",
  },
  eco2Warn: {
    l: "eCO2 warn (ppm)",
    g: "Environment",
    d: "Estimated CO2 warning",
  },
  eco2Crit: {
    l: "eCO2 critical (ppm)",
    g: "Environment",
    d: "Estimated CO2 critical",
  },
  scaleMin: { l: "Scale min (lbs)", g: "Scales", d: "Below = miscalibrated" },
  scaleMax: { l: "Scale max (lbs)", g: "Scales", d: "Above = suspicious" },
  doorSustainedMin: {
    l: "Door open alert (min)",
    g: "Doors",
    d: "Alert if open this long",
  },
  doorDailyHigh: {
    l: "Door opens/day high",
    g: "Doors",
    d: "Unusual door activity",
  },
  eventBurstPerHour: {
    l: "Event burst (/hr)",
    g: "Events",
    d: "Too many events per hour",
  },
  flatlineWindow: {
    l: "Flatline samples",
    g: "Time Series",
    d: "Identical readings to flag",
  },
  spikeZ: {
    l: "Spike z-threshold",
    g: "Time Series",
    d: "Std devs from rolling mean",
  },
  rollingN: {
    l: "Rolling window",
    g: "Time Series",
    d: "Samples for rolling stats",
  },
  memDropPct: {
    l: "Memory drop (%)",
    g: "System",
    d: "Flag if free mem drops by this %",
  },
};
...
function pointChecks(row, T, history) {
  ...
  if (ageMin > T.offlineHours * 60)
    iss.push({
      s: "critical",
      t: "offline",
      m: `No data for ${Math.round(ageMin / 60)}h`,
      g: "Connectivity",
    });
  else if (ageMin > T.staleMinutes)
    iss.push({
      s: "warning",
      t: "stale",
      m: `Last data ${fAge(ageMin)} ago`,
      g: "Connectivity",
    });
  ...
  if (b !== null) {
    ...
    if (b > 100)
      iss.push({
        s: "info",
        t: "batt_cal",
        m: `Battery ${b}% (needs calibration)`,
        g: "Power",
      });
  }

  const temp = N(row.air_temp);
  if (temp !== null && (temp < T.tempMin || temp > T.tempMax))
    iss.push({
      s: "warning",
      t: "temp",
      m: `Temp out of range: ${temp}C`,
      g: "Environment",
    });
  const hum = N(row.air_humid);
  if (hum !== null && hum > T.humidityMax)
    iss.push({
      s: "warning",
      t: "humidity",
      m: `Humidity high: ${hum}%`,
  ...
  if (pres !== null && pres > 0 && (pres < 800 || pres > 1200))
    iss.push({
      s: "warning",
      t: "pressure",
      m: `Pressure sensor fault: ${pres} hPa (expected 800-1200)`,
      g: "Environment",
    });
  ...
  if (eco2 !== null && eco2 > 0) {
    if (eco2 >= T.eco2Crit)
      iss.push({
        s: "critical",
        t: "eco2",
        m: `eCO2 high: ${eco2} ppm`,
        g: "Environment",
      });
    else if (eco2 >= T.eco2Warn)
      iss.push({
        s: "warning",
        t: "eco2",
        m: `eCO2 elevated: ${eco2} ppm`,
        g: "Environment",
      });
  }
  ...
  if (
    ...
    iss.push({
      s: "info",
      t: "food_probe",
      m: "Food temp probe disconnected (-127)",
      g: "Environment",
    });
  ...
  if (rssi !== null) {
    if (rssi < T.rssiCrit)
      iss.push({
        s: "critical",
        t: "rssi",
        m: `RSSI very weak: ${rssi} dBm`,
        g: "Connectivity",
      });
    else if (rssi < T.rssiWarn)
      iss.push({
        s: "warning",
        t: "rssi",
        m: `RSSI weak: ${rssi} dBm`,
        g: "Connectivity",
      });
  }
    ...
    if (v !== null) {
      ...
      if (v > T.scaleMax)
        iss.push({
          s: "warning",
          t: "scale_high",
          m: `Scale ${i} high: ${v} lbs`,
          g: "Scales",
        });
    }
    if (toBool(row[`scale${i}_disconnect`]))
      iss.push({
        s: "critical",
        t: "scale_disc",
        m: `Scale ${i} disconnected`,
        g: "Scales",
      });
    if (toBool(row[`scale${i}_suspect`]))
      iss.push({
        s: "warning",
        t: "scale_susp",
        m: `Scale ${i} suspect`,
  ...
  if (!toBool(row.is_event)) {
    if (toBool(row.door1_open))
      iss.push({ s: "info", t: "door", m: "Door 1 open", g: "Doors" });
    if (toBool(row.door2_open))
      iss.push({ s: "info", t: "door", m: "Door 2 open", g: "Doors" });
  }
  ...
  if (
    ...
    iss.push({
      s: "info",
      t: "bsec",
      m: "BSEC uncalibrated (accuracy=0)",
      g: "System",
    });
...
function timeSeriesChecks(history, T) {
  ...
  const spanHours =
    (new Date(sorted[len - 1].timestamp) - new Date(sorted[0].timestamp)) /
    3.6e6;
  ...
  if (gaps.length > 2) {
    ...
    if (med > 0 && last > med * T.intervalDriftX)
      iss.push({
        s: "warning",
        t: "interval_drift",
        m: `Reporting gap ${Math.round(last)}min (${(last / med).toFixed(1)}x median ${Math.round(med)}min)`,
        g: "Connectivity",
      });
  }
  ...
  for (const key of numericFields) {
    ...
    if (
      tail.length >= T.flatlineWindow &&
      tail.every((v) => v === tail[0]) &&
      key !== "batt_percent"
    )
      iss.push({
        s: "warning",
        t: "flatline",
        m: `${fKey(key)} flatlined at ${tail[0]} for ${tail.length} readings`,
        g: "Time Series",
      });
    ...
    if (std > 0.001) {
      ...
      if (z > T.spikeZ)
        iss.push({
          s: "warning",
          t: "spike",
          m: `${fKey(key)} spike: ${latest} (z=${z.toFixed(1)}, mean=${mean.toFixed(1)})`,
          g: "Time Series",
        });
    }
  }
  ...
  if (scalesZero && envZero)
    iss.push({
      s: "critical",
      t: "all_zero",
      m: "All sensors zero -- device fault or powered off",
      g: "System",
    });

  // ── Battery drain ──
  const batt = sorted
    .map((r) => ({ ts: new Date(r.timestamp).getTime(), v: N(r.batt_percent) }))
    .filter((b) => b.v !== null && b.v > 0 && b.v <= 100);
  if (batt.length >= 5) {
    ...
    if (hrs > 1) {
      ...
      if (drain > T.battDrainPerDay) {
        ...
        iss.push({
          s: drain > 15 ? "critical" : "warning",
          t: "batt_drain",
          m: `Battery draining ${drain.toFixed(1)}%/day, ~${Math.round(left)} days left`,
          g: "Power",
        });
      }
    }
  ...
  if (rssiVals.length >= 10) {
    ...
    if (avgSecond < avgFirst - 5)
      iss.push({
        s: "warning",
        t: "rssi_trend",
        m: `RSSI degrading: ${avgFirst.toFixed(0)} -> ${avgSecond.toFixed(0)} dBm over ${Math.round(spanHours)}h`,
        g: "Connectivity",
      });
  }

  // ── Memory leak ──
  const memVals = sorted
    .map((r) => N(r.memory_free))
    .filter((v) => v !== null && v > 0);
  if (memVals.length >= 10) {
    ...
    if (maxMem > 0 && ((maxMem - recentMem) / maxMem) * 100 > T.memDropPct)
      iss.push({
        s: "warning",
        t: "mem_leak",
        m: `Memory dropped ${Math.round(((maxMem - recentMem) / maxMem) * 100)}%: ${Math.round(maxMem / 1024)}KB -> ${Math.round(recentMem / 1024)}KB`,
        g: "System",
      });
  }

  // ── BSEC calibration stuck ──
  const accVals = sorted
    .slice(-20)
    .map((r) => N(r.accuracy))
    .filter((v) => v !== null);
  if (accVals.length >= 10 && accVals.every((v) => v === 0))
    iss.push({
      s: "warning",
      t: "bsec_stuck",
      m: `BSEC stuck uncalibrated for ${accVals.length} readings`,
      g: "System",
    });
    ...
    for (let d = 1; d <= 2; d++) {
      ...
      if (opensPerDay > T.doorDailyHigh)
        iss.push({
          s: "warning",
          t: "door_busy",
          m: `Door ${d}: ${Math.round(opensPerDay)} opens/day (high)`,
          g: "Doors",
        });
      ...
      if (opens.length === 0 && days > 2)
        iss.push({
          s: "info",
          t: "door_unused",
          m: `Door ${d}: no activity in ${Math.round(days)} days`,
          g: "Doors",
        });
      ...
      if (maxConsec >= 2) {
        ...
        for (let i = 0; i < sorted.length; i++) {
          if (toBool(sorted[i][`door${d}_open`])) {
            if (startIdx === -1) startIdx = i;
          } else {
            if (startIdx !== -1 && i - startIdx >= maxConsec) break;
            startIdx = -1;
          }
        }
        if (startIdx !== -1 && startIdx + maxConsec - 1 < sorted.length) {
          const durMin =
            (new Date(
              sorted[Math.min(startIdx + maxConsec - 1, len - 1)].timestamp,
            ) -
              new Date(sorted[startIdx].timestamp)) /
            60000;
          if (durMin >= T.doorSustainedMin)
            iss.push({
              s: "warning",
              t: "door_sustained",
              m: `Door ${d} open ~${Math.round(durMin)}min (${maxConsec} readings)`,
              g: "Doors",
            });
        }
      }
    ...
    if (events.length > 0) {
      ...
      if (maxPerHour > T.eventBurstPerHour)
        iss.push({
          s: "warning",
          t: "event_burst",
          m: `Event burst: ${maxPerHour} events in 1 hour`,
          g: "Events",
        });
      ...
      const midTs = new Date(
        (new Date(sorted[0].timestamp).getTime() +
          new Date(sorted[len - 1].timestamp).getTime()) /
          2,
      );
      const firstEvents = events.filter(
        (e) => new Date(e.timestamp) < midTs,
      ).length;
      ...
      if (firstEvents > 0 && secondEvents / Math.max(firstEvents, 1) > 3)
        iss.push({
          s: "info",
          t: "event_increase",
          m: `Event rate increased: ${firstEvents} -> ${secondEvents} (first vs second half)`,
          g: "Events",
        });
      if (secondEvents === 0 && firstEvents > 5)
        iss.push({
          s: "warning",
          t: "event_silence",
          m: `Events stopped: ${firstEvents} in first half, 0 in second`,
          g: "Events",
        });
    }

    // Ambient trends: temp/humidity direction over the week
    const tempVals = sorted
      .map((r) => N(r.air_temp))
      .filter((v) => v !== null && v !== 0);
    if (tempVals.length >= 20) {
      ...
      if (Math.abs(delta) > 3)
        iss.push({
          s: "info",
          t: "temp_trend",
          m: `Temp trending ${delta > 0 ? "up" : "down"}: ${earlyAvg.toFixed(1)} -> ${lateAvg.toFixed(1)}C over ${Math.round(spanHours)}h`,
          g: "Environment",
        });
    }

    // Gas resistance drift (sensor aging: decreasing resistance over time)
    const gasVals = sorted
      .map((r) => N(r.gas_resistance))
      .filter((v) => v !== null && v > 0);
      ...
      if (earlyAvg > 0) {
        ...
        if (Math.abs(changePct) > 50)
          iss.push({
            s: "info",
            t: "gas_drift",
            m: `Gas resistance ${changePct > 0 ? "increased" : "decreased"} ${Math.abs(changePct).toFixed(0)}% over ${Math.round(spanHours)}h`,
            g: "Environment",
          });
      }
    }
...
const ISSUE_CLASS = {
  ...
  scale_disc: {
    cat: "hardware",
    act: "Check scale wiring and load cell connections",
  },
  food_probe: { cat: "hardware", act: "Reconnect food temperature probe" },
  flatline: {
    cat: "hardware",
    act: "Sensor may be stuck — check wiring or replace",
  },
  spike: {
    cat: "hardware",
    act: "Check for loose wiring or electrical interference",
  },
  battery: { cat: "hardware", act: "Replace or recharge battery" },
  batt_drain: {
    cat: "hardware",
    act: "Battery draining fast — check for power-hungry firmware loop or replace battery",
  },
  // Software / firmware indicators
  mem_leak: {
    cat: "software",
    act: "Memory leak — restart device or update firmware",
  },
  bsec: {
    cat: "software",
    act: "BSEC library still calibrating — usually resolves on its own",
  },
  bsec_stuck: {
    cat: "software",
    act: "BSEC calibration stuck — restart device or update firmware",
  },
  scale_susp: {
    cat: "software",
    act: "Firmware flagged suspect reading — may need recalibration",
  },
  batt_cal: {
    cat: "software",
    act: "Battery ADC calibration off — firmware issue, not dangerous",
  },
  // Environmental
  temp: {
    cat: "environment",
    act: "Check pantry location — direct sunlight? Heating vent nearby?",
  },
  humidity: {
    cat: "environment",
    act: "Check for water ingress or poor ventilation",
  },
  iaq: {
    cat: "environment",
    act: "Poor air quality — check ventilation around pantry",
  },
  eco2: {
    cat: "environment",
    act: "Elevated CO₂ — enclosed space with poor airflow",
  },
  temp_trend: {
    cat: "environment",
    act: "Gradual temp shift — seasonal change or new heat source nearby?",
  },
  gas_drift: {
    cat: "environment",
    act: "Gas resistance shifting — sensor aging or environment change",
  },
  // Connectivity
  offline: {
    cat: "connectivity",
    act: "Device not reporting — check power and WiFi/cellular signal",
  },
  stale: {
    cat: "connectivity",
    act: "Delayed reporting — may be intermittent connectivity",
  },
  rssi: {
    cat: "connectivity",
    act: "Weak signal — move device or add antenna/repeater",
  },
  rssi_trend: {
    cat: "connectivity",
    act: "Signal degrading — check for new interference sources",
  },
  interval_drift: {
    cat: "connectivity",
    act: "Irregular reporting — connectivity drops or firmware sleep issues",
  },
  // Ambiguous (could be HW or SW)
  all_zero: {
    cat: "device",
    act: "All sensors zero — device crash or total power loss. Restart device.",
  },
  scale_neg: {
    cat: "device",
    act: "Negative scale — could be miscalibration (SW) or a shifted load cell (HW)",
  },
  scale_high: {
    cat: "device",
    act: "Very high scale reading — check for objects on scale or recalibrate",
  },
  no_data: {
    cat: "connectivity",
    act: "No data at all — verify device is powered on and network is reachable",
  },
  // Operational (not a malfunction)
  door: {
    cat: "operational",
    act: "Door is currently open — normal if someone is visiting",
  },
  door_sustained: {
    cat: "operational",
    act: "Door open a long time — may be propped for stocking",
  },
  door_busy: {
    cat: "operational",
    act: "High door activity — popular pantry!",
  },
  door_unused: {
    cat: "operational",
    act: "Door not used — check if door sensor is still attached",
  },
  event_burst: {
    cat: "operational",
    act: "Event burst — could be stocking or multiple visitors",
  },
  event_increase: {
    cat: "operational",
    act: "More activity recently — growing usage is good!",
  },
  event_silence: {
    cat: "operational",
    act: "Activity dropped off — check if door sensor is working",
  },
};
...
function diagnose(issues, history) {
  ...
  const hasFlatScale = issues.some(
    (i) => i.t === "flatline" && i.m.includes("Scale"),
  );
...
function dataQuality(history, T) {
  ...
  const ageMin =
    (Date.now() - new Date(sorted[len - 1].timestamp).getTime()) / 60000;
  const freshness = Math.max(
    0,
    Math.min(100, Math.round(100 - (ageMin / (T.offlineHours * 60)) * 100)),
  );
  ...
  const score = Math.round(
    freshness * 0.4 + completeness * 0.3 + consistency * 0.3,
  );
...
function genDemo() {
  ...
  const make = (id, intMin, bT, bB, q = {}) => {
    ...
    for (let i = count - 1; i >= 0; i--) {
      const ts = new Date(
        now - (i * intMin + (Math.random() - 0.5) * intMin * 0.2) * 60000,
      );
      ...
      const temp =
        bT +
        Math.sin(dayFrac * Math.PI * 2) * 3 +
        (Math.random() - 0.5) * 0.5 +
        (q.tempRise ? (1 - weekFrac) * 4 : 0);
      h.push({
        device_id: id,
        ...
        air_humid: +(
          60 +
          Math.sin(dayFrac * Math.PI * 2) * 15 +
          (Math.random() - 0.5) * 3
        ).toFixed(1),
        air_pressure: 10,
        gas_resistance: Math.round(
          80000 + Math.random() * 40000 + (q.gasDrift ? weekFrac * 100000 : 0),
        ),
        iaq: Math.round(
          30 +
            Math.random() * (q.iaqHigh ? 200 : 40) +
            (q.iaqHigh && i < 20 ? 150 : 0),
        ),
        ...
        accuracy: q.bsecStuck
          ? 0
          : Math.random() > 0.3
            ? 3
            : Math.floor(Math.random() * 3),
        food_temp: q.foodProbe ? +(2 + Math.random() * 3).toFixed(1) : -127,
        batt_percent: Math.max(
          0,
          Math.round(
            bB - weekFrac * (q.fastDrain ? 40 : 6) + (Math.random() - 0.5) * 2,
          ),
        ),
        batt_voltage: +(3.3 + (1 - weekFrac) * 0.8).toFixed(2),
        rssi: Math.round(
          -70 - Math.random() * 10 + (q.rssiDegrade ? -weekFrac * 15 : 0),
        ),
        memory_free: Math.round(
          110000 -
            (q.memLeak ? weekFrac * 30000 : 0) +
            (Math.random() - 0.5) * 5000,
        ),
        scale1: q.flatScale
          ? 4.57
          : +(3 + Math.random() * 4 + Math.sin(i * 0.05) * 2).toFixed(2),
        ...
        scale2_disconnect: false,
        scale3_disconnect: false,
        scale4_disconnect: false,
        scale1_suspect: false,
        scale2_suspect: !!(q.susp2 && i < 10),
        scale3_suspect: false,
        scale4_suspect: false,
        ...(q.allZero && i < 3
          ? {
              air_temp: 0,
              air_humid: 0,
  ...
  return {
    BeaconHill: make("BeaconHill", 10, 5.5, 85, {
      flatScale: true,
      tempRise: true,
    }),
    Greenwood: make("Greenwood", 30, 1.2, 18, {
      allZero: true,
      fastDrain: true,
      noDoor2: true,
      rssiDegrade: true,
    }),
    StPaulChurchPantry: make("StPaulChurchPantry", 15, 8.0, 103, {
      spikeS4: true,
      susp2: true,
      iaqHigh: true,
      eco2High: true,
    ...
    NewSite_Demo: make("NewSite_Demo", 20, 6.0, 65, {
      disc1: true,
      memLeak: true,
      gasDrift: true,
      foodProbe: true,
...
const Dot = ({ s, sz = 8 }) => (
  <span
    style={{
      ...
      boxShadow: `0 0 ${sz}px ${SC[s]}60`,
      animation: s === "critical" ? "blink 1.4s infinite" : "none",
      flexShrink: 0,
    }}
  />
);
const Tag = ({ s, children }) => (
  <span
    style={{
      ...
      color: SC[s],
      backgroundColor: SB[s],
      border: `1px solid ${SR[s]}`,
    }}
  >
    {children}
  </span>
...
const Metric = ({ label, value, unit, w }) => (
  <div
    style={{
      ...
      borderRadius: 5,
      minWidth: 75,
      backgroundColor: w ? (w === "critical" ? C.crB : C.wrB) : `${C.border}50`,
      border: `1px solid ${w ? (w === "critical" ? C.crBr : C.wrBr) : C.border}`,
    }}
  >
    <div
      style={{
    ...
    >
      {value}
      <span style={{ fontSize: 10, color: C.txM, marginLeft: 2 }}>{unit}</span>
    </div>
  </div>
);
...
function PantryCard({
  ...
  const totalW = [1, 2, 3, 4].reduce(
    (s, i) => s + (N(latest?.[`scale${i}`]) || 0),
    0,
  );
    ...
    >
      <div
        ...
        style={{
          ...
          alignItems: "center",
          gap: 10,
          background: `linear-gradient(90deg, ${SB[status]}, transparent 60%)`,
        }}
      >
        <Dot s={status} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            {label !== id && (
              <span style={{ fontSize: 11, color: C.txM }}>({id})</span>
            )}
          </div>
          <div style={{ fontSize: 11, color: C.txD }}>
            {latest?.timestamp
              ...
              : "No data"}
            {history?.length > 1 &&
              ` | ${history.length} readings | ${Math.round((new Date(history[0].timestamp) - new Date(history[history.length - 1].timestamp)) / 3.6e6)}h span`}
          </div>
        </div>
        <div
          style={{
        ...
        >
          ...
          {dq && (
            <span
              style={{
                fontSize: 10,
                fontWeight: 700,
                color: dq.score >= 70 ? C.ok : dq.score >= 40 ? C.wr : C.cr,
                fontFamily: "'DM Mono',monospace",
                padding: "1px 6px",
                borderRadius: 3,
                backgroundColor:
                  dq.score >= 70 ? C.okB : dq.score >= 40 ? C.wrB : C.crB,
              }}
            >
              Q{dq.score}
            </span>
          ...
          {maintSince && (
            <Tag s="critical">
              attn {fAge((Date.now() - new Date(maintSince).getTime()) / 60000)}
            </Tag>
          )}
        </div>
        <span
      ...
      {open && latest && (
        ...
        >
          ...
          >
            ...
            <Metric
              label="Temp"
              value={N(latest.air_temp)?.toFixed(1) || "--"}
              unit="C"
              w={
                N(latest.air_temp) < T.tempMin || N(latest.air_temp) > T.tempMax
                  ? "warning"
                  : null
              }
            />
            <Metric
              label="Humidity"
              value={N(latest.air_humid)?.toFixed(0) || "--"}
              unit="%"
              w={N(latest.air_humid) > T.humidityMax ? "warning" : null}
            />
            <Metric
              label="Battery"
              value={latest.batt_percent ?? "--"}
              unit="%"
              w={
                N(latest.batt_percent) !== null &&
                N(latest.batt_percent) <= T.battCritical
                  ? "critical"
                  : N(latest.batt_percent) !== null &&
                      N(latest.batt_percent) <= T.battLow
                    ? "warning"
                    : null
              }
            />
            <Metric
              ...
              unit="dBm"
              w={
                N(latest.rssi) < T.rssiCrit
                  ? "critical"
                  : N(latest.rssi) < T.rssiWarn
                    ? "warning"
                    : null
              }
            />
            ...
            <Metric
              label="Memory"
              value={Math.round(N(latest.memory_free || 0) / 1024)}
              unit="KB"
            />
          </div>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: 4,
                    textAlign: "center",
                    border: `1px solid ${disc ? C.crBr : susp ? C.wrBr : C.border}`,
                    backgroundColor: disc ? C.crB : susp ? C.wrB : C.bg,
                  }}
                >
                  ...
                  {disc && (
                    <div style={{ fontSize: 9, color: C.cr, fontWeight: 700 }}>
                      DISC
                    </div>
                  )}
                  {susp && (
                    <div style={{ fontSize: 9, color: C.wr, fontWeight: 700 }}>
                      SUSPECT
                    </div>
                  )}
                </div>
          ...
          >
            {[1, 2].map((i) => {
              const o = toBool(latest[`door${i}_open`]);
              return (
                <span
                  key={i}
                  style={{
                    ...
                    borderRadius: 3,
                    fontSize: 11,
                    border: `1px solid ${o ? C.wrBr : C.okBr}`,
                    color: o ? C.wr : C.ok,
                  }}
                >
                  Door {i}: {o ? "OPEN" : "Closed"}
                </span>
              );
            })}
            <span
            ...
            <div>
              {GROUPS.filter((g) => grouped[g]).map((g) => (
                <div key={g} style={{ marginBottom: 8 }}>
                  ...
                  >
                    {GROUP_ICONS[g] || ""} {g} ({grouped[g].length})
                  </div>
                  {grouped[g].map((iss, idx) => (
                    <div
                      key={idx}
                      style={{
                        padding: "5px 10px",
                        ...
                        borderRadius: 4,
                        fontSize: 12,
                        borderLeft: `3px solid ${SC[iss.s]}`,
                        color: SC[iss.s],
                        backgroundColor: SB[iss.s],
                      }}
                    >
                      <span
                        style={{
              ...
              {diag.correlation && (
                <div
                  style={{
                    ...
                    padding: "4px 8px",
                    borderRadius: 4,
                    backgroundColor: `${CAT_META[diag.correlation.cat]?.color || C.acc}10`,
                    borderLeft: `3px solid ${CAT_META[diag.correlation.cat]?.color || C.acc}`,
                  }}
                >
                  <div>{diag.correlation.msg}</div>
                  <div style={{ fontSize: 11, color: C.txD, marginTop: 2 }}>
                    → {diag.correlation.act}
                  </div>
                </div>
              )}
...
function ThreshEditor({ T, onChange, onReset }) {
  const groups = {};
  for (const [k, m] of Object.entries(TMETA)) {
    ...
    <div>
      ...
      >
        <div style={{ fontSize: 13, fontWeight: 700, color: C.tx }}>
          Threshold Settings ({Object.keys(TMETA).length})
        </div>
        <button
          onClick={onReset}
          style={{
            ...
            {items.map(({ k, l, d }) => (
              ...
              >
                ...
                <input
                  ...
                  value={T[k]}
                  onChange={(e) =>
                    onChange({ ...T, [k]: Number(e.target.value) })
                  }
                  style={{
                    width: "100%",
                    padding: "4px 6px",
                ...
                />
                <div style={{ fontSize: 9, color: C.txM, marginTop: 2 }}>
                  {d}
                </div>
              </div>
            ))}
...
function AnomalyLog({ log, nicks }) {
  ...
  const filtered = log.filter(
    (e) =>
      (filter === "all" || e.s === filter) &&
      (gFilter === "all" || e.g === gFilter),
  );
  ...
  return (
    <div>
          ...
          {["all", "critical", "warning", "info"].map((f) => (
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                textTransform: "uppercase",
                border: `1px solid ${filter === f ? C.acc : C.border}`,
                backgroundColor: filter === f ? C.accD : "transparent",
                color: filter === f ? C.acc : C.txM,
              }}
            >
              {f}
            </button>
          ))}
          <span style={{ color: C.txM, fontSize: 11, padding: "2px 4px" }}>
            |
          </span>
          {["all", ...allGroups].map((f) => (
            <button
              ...
              style={{
                ...
                fontSize: 10,
                cursor: "pointer",
                border: `1px solid ${gFilter === f ? C.acc : C.border}`,
                backgroundColor: gFilter === f ? C.accD : "transparent",
                color: gFilter === f ? C.acc : C.txM,
              }}
            >
              {f === "all" ? "All Groups" : f}
            </button>
      ...
      {filtered.length === 0 ? (
        <div style={{ padding: 30, textAlign: "center", color: C.txM }}>
          No matching entries.
        </div>
      ) : (
        <div
          ...
          >
            <thead>
              <tr style={{ backgroundColor: C.card }}>
                {["Time", "Pantry", "Sev", "Group", "Type", "Details"].map(
                  (h) => (
                    <th
                      key={h}
                      style={{
            ...
            <tbody>
              {filtered.slice(0, 300).map((e, i) => (
                <tr key={i} style={{ borderBottom: `1px solid ${C.border}20` }}>
                  ...
                  <td
                    style={{ padding: "5px 8px", color: C.txD, fontSize: 10 }}
                  >
                    {e.g}
                  </td>
...
function ChartFrame({ children, H, yLabels, timeData, note, legend }) {
  ...
  const t0str = timeData?.[0]?.timestamp
    ? new Date(timeData[0].timestamp).toLocaleDateString(undefined, {
        month: "short",
        day: "numeric",
      })
    : "";
...
function ScaleChart({ history, T }) {
  ...
  if (data.length < 2)
    return (
      <div
        style={{ color: C.txM, fontSize: 11, padding: 20, textAlign: "center" }}
      >
        Not enough data
      </div>
    );
  const allVals = data.flatMap((d) =>
    [1, 2, 3, 4].map((i) => N(d[`scale${i}`])).filter((v) => v !== null),
  );
  ...
  return (
    <ChartFrame
      H={H}
      yLabels={[`${mx.toFixed(0)}`, `${mid.toFixed(0)}`, `${mn.toFixed(0)}`]}
      timeData={data}
      note={`${data.length} readings`}
      legend={
        <>
          {[1, 2, 3, 4].map((i) => (
            <span key={i} style={{ color: SCALE_COLORS[i - 1] }}>
              ● S{i}
            </span>
          ))}
    ...
    >
      ...
      {(() => {
        const d = tsPath(
          data,
          (r) => [1, 2, 3, 4].reduce((s, i) => s + (N(r[`scale${i}`]) || 0), 0),
          W,
          H,
          t0,
          t1,
...
function DoorTimeline({ history }) {
  ...
  function getRuns(doorKey) {
    ...
    for (const r of data) {
      if (toBool(r[doorKey])) {
        ...
        if (start !== null) {
          runs.push({ start, end: new Date(r.timestamp).getTime() });
          start = null;
        }
      }
    }
  ...
  const runs1 = getRuns("door1_open"),
    runs2 = getRuns("door2_open");
...
function BatteryChart({ history, T }) {
  ...
  const lineColor =
    latestB !== null && latestB <= T.battCritical
      ? C.cr
      : latestB !== null && latestB <= T.battLow
        ? C.wr
        : C.ok;
  ...
  return (
    ...
    >
      ...
      <RefLine
        ...
        color={C.wr}
        W={W}
        label={`warn ${T.battLow}%`}
      />
      <RefLine
        y={toY(T.battCritical)}
        color={C.cr}
        W={W}
        label={`crit ${T.battCritical}%`}
      />
      {pathD && (
        <path
          d={pathD}
...
function DiagnosticsTab({ analysis, nicks, T }) {
  ...
  const firstWithData =
    devs.find((id) => (analysis[id]?.history?.length ?? 0) > 1) ||
    devs[0] ||
    "";
  ...
  useEffect(() => {
    if ((!sel || !analysis[sel]) && devs.length)
      setSel(
        devs.find((id) => (analysis[id]?.history?.length ?? 0) > 1) || devs[0],
      );
  }, [devs, analysis, sel]);
  const dev = analysis[sel];
  const sdIssues =
    dev?.issues.filter((i) => i.g === "Scales" || i.g === "Doors") || [];
  const otherIssues =
    dev?.issues.filter((i) => i.g !== "Scales" && i.g !== "Doors") || [];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      >
        ...
        {dev && (
          <span style={{ fontSize: 11, color: C.txM }}>
            {dev.history.length} readings · {dev.issues.length} findings
          </span>
        )}
      </div>
      {dev && (
        <>
            ...
            <>
              ...
              >
                {[
                  ...
                  {
                    l: "Completeness",
                    v: dev.dq.completeness,
                    clr:
                      dev.dq.completeness >= 70
                        ? C.ok
                        : dev.dq.completeness >= 40
                          ? C.wr
                          : C.cr,
                  },
                  {
                    l: "Consistency",
                    v: dev.dq.consistency,
                    clr:
                      dev.dq.consistency >= 70
                        ? C.ok
                        : dev.dq.consistency >= 40
                          ? C.wr
                          : C.cr,
                  },
                ].map((g) => (
              ...
              {dev.diag.correlation && (
                <div
                  style={{
                    ...
                    borderRadius: 6,
                    marginBottom: 12,
                    backgroundColor: `${CAT_META[dev.diag.correlation.cat]?.color || C.acc}10`,
                    border: `1px solid ${CAT_META[dev.diag.correlation.cat]?.color || C.acc}30`,
                  }}
                >
                  <div
                    style={{
                      ...
                      fontWeight: 700,
                      textTransform: "uppercase",
                      color: CAT_META[dev.diag.correlation.cat]?.color || C.acc,
                      marginBottom: 4,
                    }}
                  >
                    ⚡ Cross-Sensor Correlation
                  ...
                  <div style={{ fontSize: 12, color: C.tx, marginBottom: 4 }}>
                    {dev.diag.correlation.msg}
                  </div>
                  <div style={{ fontSize: 11, color: C.txD }}>
                    → {dev.diag.correlation.act}
                  </div>
                </div>
              )}
              ...
              {Object.keys(dev.diag.cats).length > 0 ? (
                ...
                <div style={{ fontSize: 12, color: C.ok, padding: 10 }}>
          ...
          {panel(
            "⚖ Scale Readings (lbs)",
            <ScaleChart history={dev.history} T={T} />,
          )}
          {panel("🚪 Door Events", <DoorTimeline history={dev.history} />)}
          {panel(
            "🔋 Battery Trend",
            <BatteryChart history={dev.history} T={T} />,
          )}
      ...
      {!dev && (
        <div style={{ padding: 40, textAlign: "center", color: C.txM }}>
          No pantry selected
        </div>
      )}
    </div>
...
function DataExplorerTab({ analysis, nicks }) {
  const devs = Object.keys(analysis);
  const firstWithData =
    devs.find((id) => (analysis[id]?.history?.length ?? 0) > 0) ||
    devs[0] ||
    "";
  const [sel, setSel] = useState(firstWithData);
  useEffect(() => {
    if ((!sel || !analysis[sel]) && devs.length)
      setSel(
        devs.find((id) => (analysis[id]?.history?.length ?? 0) > 0) || devs[0],
      );
  }, [devs, analysis, sel]);
  ...
  const scaleCell = (row, i) => {
    ...
    return (
      <TD
        ...
        mono
        right
        color={disc ? C.cr : susp ? C.wr : C.tx}
        bg={disc ? C.crB : susp ? C.wrB : undefined}
      >
        {disc
          ? "DISC"
          : susp
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      >
        ...
        {rows.length > 0 && (
          <span style={{ fontSize: 11, color: C.txM }}>
            {rows.length} readings · newest first
          </span>
        )}
      </div>
      {rows.length > 0 ? (
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {rows.map((r, i) => {
                  ...
                  const d1 = toBool(r.door1_open),
                    d2 = toBool(r.door2_open);
                  const b = N(r.batt_percent),
                    rssi = N(r.rssi);
                  return (
                    <tr
                      key={i}
                    ...
                    >
                      ...
                      <TD mono right color={C.acc}>
                        {total.toFixed(2)}
                      </TD>
                      <TD right color={d1 ? C.wr : C.txM}>
                        {d1 ? "OPEN" : "–"}
                      </TD>
                      <TD right color={d2 ? C.wr : C.txM}>
                        {d2 ? "OPEN" : "–"}
                      </TD>
                      <TD mono right>
                        {N(r.air_temp)?.toFixed(1) ?? "--"}
                      </TD>
                      <TD mono right>
                        {N(r.air_humid)?.toFixed(0) ?? "--"}
                      </TD>
                      <TD
                        mono
                        right
                      ...
                      <TD
                        mono
                        right
                        color={rssi < -90 ? C.cr : rssi < -85 ? C.wr : C.tx}
                      >
                        {rssi ?? "--"}
                      </TD>
                    </tr>
...
function SimplePantryCard({ id, dev, nicks }) {
  ...
  const totalW = latest
    ? [1, 2, 3, 4].reduce((s, i) => s + (N(latest[`scale${i}`]) || 0), 0)
    : 0;
  const ageMin = latest
    ? (Date.now() - new Date(latest.timestamp).getTime()) / 60000
    : null;
  const recentOpens = dev.history
    .slice(0, 48)
    .filter((r) => toBool(r.door1_open) || toBool(r.door2_open)).length;
  const insights = dev.issues
    .filter((i) => i.t !== "no_data")
    .slice(0, 2)
    .map(friendlyInsight);
  return (
    <div
    ...
    >
      ...
      >
        <div>
          ...
          {label !== id && (
            <div style={{ fontSize: 10, color: C.txM, marginTop: 1 }}>{id}</div>
          )}
        </div>
...
export default function PantryMonitor() {
  ...
  const [apiBase, setApiBase] = useState(
    sGet(
      "pm-a",
      "https://pantryapi-web-d8gzfkftgtb5cfhn.westus2-01.azurewebsites.net",
    ),
  );
  ...
  useEffect(() => {
    const t = sGet("pm-t", null);
    if (t) setT(t);
    const n = sGet("pm-n", null);
    if (n) setNicks(n);
    const w = sGet("pm-w", null);
    if (w) setWebhook(w);
    const l = sGet("pm-l", null);
    if (l) setLog(l);
  }, []);
  useEffect(() => {
    sSet("pm-mode", mode);
  ...
  useEffect(() => {
    if (log.length > 0) sSet("pm-l", log.slice(0, 1000));
  }, [log]);

  const onNick = useCallback(
    (id, name) => setNicks((p) => ({ ...p, [id]: name || id })),
    [],
  );
  ...
  const fetchLive = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `${apiBase.replace(/\/$/, "")}/api/GetLatestPantry?pantryId=all&mode=monitor`,
        { cache: "no-store" },
      );
  ...
  useEffect(() => {
    if (mode === "live" && apiBase) {
      fetchLive();
      const iv = setInterval(fetchLive, 5 * 60000);
      return () => clearInterval(iv);
    }
  }, [mode, apiBase, fetchLive]);
  ...
  const analysis = useMemo(() => {
    ...
    for (const [dev, hist] of Object.entries(pH)) {
      ...
      const pt = latest
        ? pointChecks(latest, T, sorted)
        : [{ s: "critical", t: "no_data", m: "No data", g: "Connectivity" }];
  ...
  useEffect(() => {
    ...
    for (const [dev, { issues, status }] of Object.entries(analysis)) {
      if (status === "critical" || status === "warning") {
        if (!maint.current[dev]) maint.current[dev] = new Date().toISOString();
      } else delete maint.current[dev];
      for (const iss of issues) {
        const k = `${dev}-${iss.t}-${iss.m.slice(0, 40)}`;
        if (!seen.current.has(k)) {
          seen.current.add(k);
          entries.push({ at: new Date().toISOString(), dev, ...iss });
        }
      }
    }
    if (entries.length > 0) {
  ...
  const sortedDevs = useMemo(() => {
    ...
    const sdScore = (dev) => {
      ...
      if (
        iss.some(
          (i) => (i.g === "Scales" || i.g === "Doors") && i.s === "critical",
        )
      )
        return 0;
      if (
        iss.some(
          (i) => (i.g === "Scales" || i.g === "Doors") && i.s === "warning",
        )
      )
        return 1;
      return 2;
  ...
  const critN = Object.values(analysis).filter(
    (r) => r.status === "critical",
  ).length;
  const warnN = Object.values(analysis).filter(
    (r) => r.status === "warning",
  ).length;
  const totalChecks = Object.values(analysis).reduce(
    ...
    >
      ...
      >
        ...
        >
          {simpleView ? (
            <>
              {critN > 0 && (
                <span style={{ fontSize: 12, color: "#e3a008" }}>
                  🔔 {critN} need attention
                </span>
              )}
              {critN === 0 && warnN === 0 && sortedDevs.length > 0 && (
            ...
            <>
              ...
              {critN === 0 && warnN === 0 && sortedDevs.length > 0 && (
                <Tag s="ok">all clear</Tag>
              )}
            </>
          )}
          <button
            onClick={() => (mode === "live" ? fetchLive() : loadDemo())}
            disabled={loading}
            style={{
              padding: "5px 12px",
              borderRadius: 5,
          ...
          >
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                border: "none",
                backgroundColor: simpleView ? C.accD : "transparent",
                color: simpleView ? C.acc : C.txD,
                fontWeight: simpleView ? 700 : 400,
              }}
            >
              Overview
            </button>
      ...
      >
        {[
          ...
          { id: "charts", l: "Diagnostics" },
          { id: "explorer", l: "Data" },
          { id: "log", l: `Log (${log.length})` },
      ...
      {tab === "dashboard" && (
        <>
          {sortedDevs.length === 0 && (
            <div style={{ padding: 40, textAlign: "center", color: C.txM }}>
              No pantries.{" "}
              {mode === "demo" ? "Loading..." : "Check API URL in Settings."}
            </div>
          )}
          {simpleView ? (
            <div
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {sortedDevs.map((id) => (
                <PantryCard
                  key={id}
                  id={id}
                  latest={analysis[id].latest}
                  history={analysis[id].history}
                  issues={analysis[id].issues}
                  maintSince={maint.current[id]}
                  T={T}
                  nicks={nicks}
                  onNick={onNick}
                  dq={analysis[id].dq}
                  diag={analysis[id].diag}
                />
              ))}
              <div
                style={{
              ...
              >
                ...
                >
                  Check Coverage ({Object.keys(TMETA).length} thresholds across{" "}
                ...
                >
                  <div>
                    <span style={{ color: C.acc }}>Staleness/Offline</span> --
                    no data beyond expected window
                  </div>
                  <div>
                    <span style={{ color: C.acc }}>RSSI level + trend</span> --
                    weak signal, degradation over time
                  </div>
                  <div>
                    <span style={{ color: C.acc }}>Interval drift</span> --
                    reporting gap vs median cadence
                  </div>
                  <div>
                    <span style={{ color: C.acc }}>Battery level + drain</span>{" "}
                    -- thresholds + projected days left
                  </div>
                  <div>
                    <span style={{ color: C.acc }}>Temp/Humidity range</span> --
                    out-of-bounds environment
                  </div>
                  <div>
                    <span style={{ color: C.acc }}>IAQ + eCO2</span> -- air
                    quality index and estimated CO2
                  </div>
                  <div>
                    <span style={{ color: C.acc }}>Gas resistance drift</span>{" "}
                    -- sensor aging over the week
                  </div>
                  <div>
                    <span style={{ color: C.acc }}>Temp trend</span> --
                    rising/falling over analysis window
                  </div>
                  <div>
                    <span style={{ color: C.acc }}>Food probe status</span> --
                    -127 = disconnected
                  </div>
                  <div>
                    <span style={{ color: C.acc }}>
                      Scale bounds/disc/suspect
                    </span>{" "}
                    -- range, hardware, firmware flags
                  </div>
                  <div>
                    <span style={{ color: C.acc }}>Flatline detection</span> --
                    identical readings across any sensor
                  </div>
                  <div>
                    <span style={{ color: C.acc }}>Spike (z-score)</span> --
                    outliers on any numeric field
                  </div>
                  <div>
                    <span style={{ color: C.acc }}>All-zero correlation</span>{" "}
                    -- multi-sensor zero = device fault
                  </div>
                  <div>
                    <span style={{ color: C.acc }}>Door open patterns</span> --
                    frequency, sustained open, unused doors
                  </div>
                  <div>
                    <span style={{ color: C.acc }}>Event rate</span> -- bursts,
                    silence, trend shifts
                  </div>
                  <div>
                    <span style={{ color: C.acc }}>Memory trend</span> -- free
                    memory drop (leak detection)
                  </div>
                  <div>
                    <span style={{ color: C.acc }}>BSEC calibration</span> --
                    accuracy stuck at 0
                  </div>
                  <div>
                    <span style={{ color: C.acc }}>Battery calibration</span> --
                    readings over 100%
                  </div>
                </div>
              </div>
      ...
      {/* Diagnostics */}
      {tab === "charts" && (
        <DiagnosticsTab analysis={analysis} nicks={nicks} T={T} />
      )}

      {/* Data Explorer */}
      {tab === "explorer" && (
        <DataExplorerTab analysis={analysis} nicks={nicks} />
      )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            >
              <div style={{ flex: "1 1 280px" }}>
                ...
                <input
                  ...
                  placeholder="https://pantryapi-web-d8gzfkftgtb5cfhn.westus2-01.azurewebsites.net"
                  style={{
                    width: "100%",
                    padding: "7px 10px",
                    borderRadius: 5,
            ...
            <div style={{ display: "flex", gap: 6 }}>
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${mode === "live" ? C.ok : C.border}`,
                  backgroundColor: mode === "live" ? C.okB : "transparent",
                  color: mode === "live" ? C.ok : C.txD,
                  opacity: apiBase ? 1 : 0.4,
                }}
              >
                Go Live
              </button>
              <button
                ...
                style={{
                  ...
                  fontSize: 12,
                  cursor: "pointer",
                  border: `1px solid ${mode === "demo" ? C.acc : C.border}`,
                  backgroundColor: mode === "demo" ? C.accD : "transparent",
                  color: mode === "demo" ? C.acc : C.txD,
                }}
              >
                Demo Mode
              </button>
          ...
          >
            ...
            <div style={{ fontSize: 11, color: C.txM, marginBottom: 8 }}>
              Pantries auto-discovered from API. New devices appear on next
              refresh.
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {Object.keys(pH).map((id) => (
                <div
                  key={id}
                ...
                >
                  ...
                  <input
                    value={nicks[id] || ""}
                    onChange={(e) => onNick(id, e.target.value)}
                    placeholder={id}
                    style={{
                      flex: 1,
                      padding: "4px 8px",
          ...
          >
            ...
            <div style={{ fontSize: 12, color: C.txD, lineHeight: 1.6 }}>
              <p style={{ marginBottom: 6 }}>
                ...
                >
                  GET /api/GetLatestPantry?pantryId=all&mode=monitor
                </code>
              </p>
              <p style={{ marginBottom: 6 }}>
                Returns 1 week of history per device. Existing callers without{" "}
                <code
                  style={{
                    color: C.acc,
                    fontFamily: "'DM Mono',monospace",
                ...
                get the same response as before (single latest record).
              </p>
              <p>
                CORS:{" "}
                <code
      ...
      >
        Pantry Monitor v3 | 30-day analysis window | {Object.keys(TMETA).length}{" "}
        thresholds | 18 check types across {GROUPS.length} domains |
        auto-refresh 5min
      </div>
    </div>
  );
